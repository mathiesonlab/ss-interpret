...
pops = ["CEU", "CHB", "YRI"]
model_strs = ["discs/{pop}/disc_N.keras", "models/{pop}/{pop}_N_230410.h5"]
fc_sizes = [64, 128]
...
def main_preds():
    for pop in pops:
        ...
        for model_str, fc_size in zip(model_strs, fc_sizes):
            model_str = model_str.format(pop=pop)
            for seed in iterate_seeds(model_str):
                accs[seed] = {"overall": 0}
            ...
            accs_df.to_csv(f"./figdata/discriminator_accuracy_{pop}_{model_prefix}.csv")

def main_viz():
    for pop in pops:
        # also a stacked figure
        fig, axs = plt.subplots(2, 1, figsize=(8, 12))
        for i, model_str in enumerate(model_strs):
            model_str = model_str.format(pop=pop)
            model_prefix = model_str.split("N")[0].rsplit("/")[0].replace("-", "").replace("_", "")
            accs_df = pd.read_csv(f"./figdata/discriminator_accuracy_{pop}_{model_prefix}.csv", index_col=0)
